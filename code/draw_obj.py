...
class Draw_Obj:
    ...
    def Draw(self, output, gpos, xxx_todo_changeme1):
        (sx, sy) = xxx_todo_changeme1
...
def Make_Cache_Item(key):
    ...
    class Real_Draw_Obj:
        def __init__(self, key):
            ...
            new_width = math.floor(Get_Grid_Size() * grid_size)
            new_height = math.floor(( new_width * h ) / w)
        ...
        def Draw(self, output, gpos, xxx_todo_changeme):
            ...
            x += self.offset_x - sx
            y += self.offset_y - sy
            output.blit(self.frames[ math.floor( frame / 2 ) % len(self.frames) ], (x,y))
