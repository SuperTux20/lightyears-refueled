...
def Main_Loop(screen, clock, xxx_todo_changeme,
    ...
    while (loop_running):
        ...
        if (g.challenge == MENU_TUTORIAL):
            ...
            until_next = [
                ((128, 128, 128), 12, f"{math.floor((g.season_ends - cur_time) + 1)} days until next season)")]
        ...
        if (g.net.hub.Get_Pressure() < PRESSURE_DANGER):
            ...
            if (g.game_ends_at == None):
                ...
                New_Mail(
                    f"Game will end on Day {g.game_ends_at} unless supplies are increased.", (255, 0, 0))
        ...
        if (((not tutor.Permit_Season_Change()) and (g.season == SEASON_QUIET)) or (g.challenge == MENU_PEACEFUL)):
        ...
        if (g.season_ends <= cur_time):
            ...
            if (g.challenge != MENU_PEACEFUL):
                New_Mail(
                    f"The {g.season_fx.name} season has started.", (200, 200, 200))
        ...
        elif ((g.net.hub.tech_level >= DIFFICULTY.CITY_MAX_TECH_LEVEL) and (g.game_running)):
            # Game over - you win!
            g.game_running = False
            g.win = True
        ...
        if (g.historian_time <= cur_time) and (g.game_running) and (not menu_inhibit):
